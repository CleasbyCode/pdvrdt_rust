...
fn build_iccp_chunk(profile_vec: &[u8]) -> Result<Vec<u8>> {
    ...
    let chunk_data_size = checked_chunk_data_size(profile_vec.len(), SIZE_DIFF)? as usize;
...
fn insert_reddit_padding(png_vec: &mut Vec<u8>) -> Result<()> {
    ...
    if png_vec.len() < INSERT_INDEX_DIFF {
        bail!("Image File Error: PNG metadata is too short for Reddit padding.");
    }

    png_vec.reserve(reddit_chunk.len());
    let insert_pos = png_vec.len() - INSERT_INDEX_DIFF;
    png_vec.splice(insert_pos..insert_pos, reddit_chunk);
    Ok(())
}
...
pub fn conceal_data(png_vec: &mut Vec<u8>, option: Option_, data_file_path: &Path) -> Result<()> {
    ...
    if data_vec.len() > usize::MAX.saturating_sub(png_vec.len()) {
    ...
    if is_mastodon {
        ...
        png_vec.splice(MASTODON_INSERT_INDEX..MASTODON_INSERT_INDEX, iccp_chunk);

        twitter_iccp_compatible = png_vec.len() <= TWITTER_IMAGE_MAX_SIZE
        ...
        png_vec.splice(insert_index..insert_index, idat_chunk);
    }
